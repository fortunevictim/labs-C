...
int main()
{
    ...
    int b1, b2, lenB1Not, b1Period, b2Period, i, nPeriod = 0, frac = 0,l=0;
    unsigned long long intPart, fracPart, del=1;
    int scannedCount = scanf("%d%d%100s", &b1, &b2, b1Not);
    if (scannedCount != 3) {
    ...
    if (nPeriod > 1 || b1Not[0] == '.' || b1Not[lenB1Not-1] == '.' || i < lenB1Not
        || b1 < 2 || 16 < b1 || b2 < 2 || 16 < b2) {
        printf("bad input");
        return 0;
    }
    if (nPeriod == 0) {
    ...
    intPart = 0;
    for (i = 0; i < b1Period; i++)
        intPart = intPart*b1+(strchr(digits, tolower(b1Not[i])) - digits); 
    fracPart = 0;
    ...
    b2Period=0;
    while (intPart != 0) {
        unsigned long long f =intPart%b2;
    ...
    if (b2Period == 0) {
        ...
        if (frac) {
            printf(".");
            while (fracPart > 0 && l<12) {
                fracPart = fracPart * b2;
                l++;
                unsigned long long c = fracPart / del;
